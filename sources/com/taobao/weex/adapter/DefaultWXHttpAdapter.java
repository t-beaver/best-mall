    ...
    public void sendRequest(final WXRequest wXRequest, final IWXHttpAdapter.OnHttpListener onHttpListener) {
        ...
        execute(new Runnable() {
            public void run() {
                ...
                boolean z2 = false;
                try {
                    HttpURLConnection access$100 = DefaultWXHttpAdapter.this.openConnection(wXRequest, onHttpListener);
                    eventReporterDelegate.preConnect(access$100, wXRequest.body);
                    Map headerFields = access$100.getHeaderFields();
                    ...
                    if (responseCode < 200 || responseCode > 299) {
                        wXResponse.errorMsg = DefaultWXHttpAdapter.this.readInputStream(access$100.getErrorStream(), onHttpListener);
                        z = false;
                    } else {
                        wXResponse.originalData = DefaultWXHttpAdapter.this.readInputStreamAsBytes(eventReporterDelegate.interpretResponseStream(access$100.getInputStream()), onHttpListener);
                    }
