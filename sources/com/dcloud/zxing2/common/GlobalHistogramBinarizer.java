...
public class GlobalHistogramBinarizer extends Binarizer {
    ...
    public BitMatrix getBlackMatrix() throws NotFoundException {
        ...
        for (int i = 1; i < 5; i++) {
            ...
            for (int i3 = width / 5; i3 < i2; i3++) {
                int i4 = (row[i3] & IApp.ABS_PRIVATE_WWW_DIR_APP_MODE) >> 3;
                iArr[i4] = iArr[i4] + 1;
            }
        }
        ...
        for (int i5 = 0; i5 < height; i5++) {
            ...
            for (int i7 = 0; i7 < width; i7++) {
                if ((matrix[i6 + i7] & IApp.ABS_PRIVATE_WWW_DIR_APP_MODE) < estimateBlackPoint) {
                    bitMatrix.set(i7, i5);
                }
            }
        }
    ...
    public BitArray getBlackRow(int i, BitArray bitArray) throws NotFoundException {
        ...
        for (int i2 = 0; i2 < width; i2++) {
            int i3 = (row[i2] & IApp.ABS_PRIVATE_WWW_DIR_APP_MODE) >> 3;
            iArr[i3] = iArr[i3] + 1;
        }
        int estimateBlackPoint = estimateBlackPoint(iArr);
        byte b = row[0] & IApp.ABS_PRIVATE_WWW_DIR_APP_MODE;
        int i4 = 1;
        byte b2 = b;
        byte b3 = row[1] & IApp.ABS_PRIVATE_WWW_DIR_APP_MODE;
        while (i4 < width - 1) {
            int i5 = i4 + 1;
            byte b4 = row[i5] & IApp.ABS_PRIVATE_WWW_DIR_APP_MODE;
