...
public class ByteBufferGifDecoder implements ResourceDecoder<ByteBuffer, GifDrawable> {
    ...
    private static int getSampleSize(GifHeader gifHeader, int i, int i2) {
        ...
        if (Log.isLoggable(TAG, 2) && max > 1) {
            Log.v(TAG, "Downsampling GIF, sampleSize: " + max + ", target dimens: [" + i + Constants.Name.X + i2 + "], actual dimens: [" + gifHeader.getWidth() + Constants.Name.X + gifHeader.getHeight() + Operators.ARRAY_END_STR);
        }
        return max;
    }
