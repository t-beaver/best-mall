...
public class WebpUtil {
    ...
    private static Pair<Integer, Integer> getVP8LDimension(InputStream inputStream) throws IOException {
        ...
        byte read = ((byte) inputStream.read()) & IApp.ABS_PRIVATE_WWW_DIR_APP_MODE;
        byte read2 = ((byte) inputStream.read()) & IApp.ABS_PRIVATE_WWW_DIR_APP_MODE;
        return new Pair<>(Integer.valueOf((read | ((read2 & 63) << 8)) + 1), Integer.valueOf(((((((byte) inputStream.read()) & IApp.ABS_PRIVATE_WWW_DIR_APP_MODE) & 15) << 10) | ((((byte) inputStream.read()) & IApp.ABS_PRIVATE_WWW_DIR_APP_MODE) << 2) | ((read2 & 192) >> 6)) + 1));
    }
    ...
    private static int getInt(InputStream inputStream) throws IOException {
        return ((((byte) inputStream.read()) << 24) & -16777216) | ((((byte) inputStream.read()) << 16) & 16711680) | ((((byte) inputStream.read()) << 8) & 65280) | (((byte) inputStream.read()) & IApp.ABS_PRIVATE_WWW_DIR_APP_MODE);
    }

    public static int get2BytesAsInt(InputStream inputStream) throws IOException {
        return ((((byte) inputStream.read()) << 8) & 65280) | (((byte) inputStream.read()) & IApp.ABS_PRIVATE_WWW_DIR_APP_MODE);
    }

    private static int read3Bytes(InputStream inputStream) throws IOException {
        byte b = getByte(inputStream);
        return ((getByte(inputStream) << 16) & 16711680) | ((getByte(inputStream) << 8) & 65280) | (b & IApp.ABS_PRIVATE_WWW_DIR_APP_MODE);
    }
